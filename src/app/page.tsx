...
import SmallStockChart from './components/charts/SmallStockChart';
import TrendingSmallCaps from './components/stock-banner/TrendingSmallCaps';
import InFocus from './components/stock-banner/InFocus';
import Link from 'next/link';
import CommoditiesGrid from './components/stock-banner/CommoditiesGrid';
import YieldsGrid from './components/stock-banner/YieldsGrid';
import ValuationsGrid from './components/stock-banner/ValuationsGrid';
import VolatilityGrid from './components/stock-banner/VolatilityGrid';
import ForexGrid from './components/stock-banner/ForexGrid';
import CryptoGrid from './components/stock-banner/CryptoGrid';
import TrendingDividendStocks from './components/stock-banner/TrendingDividendStocks';
import TrendingAIStocks from './components/stock-banner/TrendingAIStocks';
...
export default function HomePage() {
  ...
  const [weeklyFocusArticle, setWeeklyFocusArticle] = useState<Article | null | undefined>(undefined);
  const [featuredAnalysisArticle, setFeaturedAnalysisArticle] = useState<Article | null | undefined>(undefined);

  const [latestNews, setLatestNews] = useState<NewsItem[] | undefined>(undefined);

  useEffect(() => {
    const fetchContentfulArticles = async () => {
      setWeeklyFocusArticle(undefined);
      setFeaturedAnalysisArticle(undefined);
      ...
      try {
        ...
        setWeeklyFocusArticle(wfData);
        setFeaturedAnalysisArticle(faData);
        setLatestNews(newsData);

      } catch (error) {
        console.error("Failed to fetch articles:", error);
        setWeeklyFocusArticle(null);
        setFeaturedAnalysisArticle(null);
      }
    };
...
const weeklyFocusTitle = weeklyFocusArticle === undefined
    ? 'Loading Weekly Focus...'
    : (weeklyFocusArticle ? weeklyFocusArticle.headline : 'No Weekly Focus Article Published');

  const featuredAnalysisLink = featuredAnalysisArticle ? `/articles/${featuredAnalysisArticle.slug}` : '#';
const featuredAnalysisTitle = featuredAnalysisArticle === undefined
    ? 'Loading Featured Analysis...'
    : (featuredAnalysisArticle ? featuredAnalysisArticle.headline : 'No Featured Analysis Article Published');

const renderNews = () => {
  if (latestNews === undefined) {
    return <div className="p-2 text-sm text-gray-500">Loading latest news...</div>;
  }
        ...
        <div className="mt-1 mr-8 flex flex-col items-center">
          <div className="flex space-x-1 mb-2">
            ...
            <button
              ...
              className={`px-3 py-2 text-[10px] flex felx-col rounded transition-colors cursor-pointer ${
        ...
        <div className="flex flex-col h-full border-l">
          ...
          <div className="flex flex-col gap-0">
            <Link href="/markets/commodities">
              <div className="bg-bgray-400 border border-black rounded p-1 m-1 hover:bg-gray-300">
            ...
            <div className="flex flex-col gap-0">
            <div className="bg-bgray-400 border border-black rounded p-1 m-1">
              <h5 className="text-[14px] text-bold font-serif text-center">
                Treausury Yields
              </h5>
            </div>
      ...
      <div className="flex-1 h-full bg-gray-200 m-2 rounded flex flex-col">
        ...
        <div className="ml-2 mr-2 mb-2 rounded flex-1 grid grid-cols-5 gap-1 p-2 overflow-y-auto bg-gray-300">
            <Link href="stocks/stock-screener">
              <div className="bg-white rounded p-2 text-center birder border-gray-800 cursor-pointer">
                <h5 className="text-[20px] font-bold text-black mb-1 bg-gray-200 rounded p-1 hover:bg-gray-300">
                  Stock Screener
                  </h5>
                  <p className="text-[14px] text-black">
                    Use out stock screener to find the best opportunities in the market today
                  </p>
              </div>
            </Link>
            <Link href="tools/dcf-calculator">
              <div className="bg-white rounded p-2 text-center birder border-gray-800 cursor-pointer">
                <h5 className="text-[20px] font-bold text-black mb-1 bg-gray-200 rounded p-1 hover:bg-gray-300">
                  ...
                  <p className="text-[14px] text-black">
                    Out Discounted Cash Flow calculator let's you see the true value of a company in seconds
                  </p>
              </div>
            </Link>
